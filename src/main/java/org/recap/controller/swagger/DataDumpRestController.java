...
public class DataDumpRestController {
    ...
    public ResponseEntity exportDataDump(@ApiParam(value = "Code of institutions whose shared collection updates are requested. Use PUL for Princeton, CUL for Columbia and NYPL for NYPL." , required = true, name = "institutionCodes") @RequestParam String institutionCodes,
                                         ...
                                         @ApiParam(value = "Type of format - Marc xml (use 0) or SCSB xml (use 1)",required=true, name = "outputFormat") @RequestParam String outputFormat,
                                         @ApiParam(value = "Type of export - Full (use 0) or Incremental (use 1)" , required = true , name = "fetchType") @RequestParam String fetchType,
        ...
        dataDumpPreProcessorService.setDataDumpRequest(dataDumpRequest,fetchType,institutionCodes,date,collectionGroupIds,transmissionType,requestingInstitutionCode,noOfRecordsPerFile,outputFormat);
